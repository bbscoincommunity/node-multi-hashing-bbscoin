...
void cryptonight_lite(const FunctionCallbackInfo<Value>& args) {
    bool fast = false;
    ...
    if(fast)
        ...
        if (cn_variant > 0 && input_len < 43) {
            except("Argument must be 43 bytes for monero variant 1+");
            return;
        }
...
void init(Local<Object> exports) {
    ...
    NODE_SET_METHOD(exports, "cryptonight_lite", cryptonight_lite);
